...
from config import (
    RATE_LIMIT_CRAWL, HTTP_TIMEOUT_CRAWL, MAX_CONTENT_CHARS, MAX_STRATEGIC_LINKS,
    CRAWL_ANALYZE_MAX_RETRIES, CRAWL_ANALYZE_RETRY_DELAY, CRAWL_ANALYZE_MAX_TOKENS,
    LLM_MODEL,
)
...
def _crawl_single_page(url: str) -> str:
    ...
    try:
        _rate_limit_crawl()
        resp = requests.get(url, headers=HEADERS, timeout=HTTP_TIMEOUT_CRAWL, allow_redirects=True)
...
def _find_strategic_links(html_content: str, base_url: str) -> list[str]:
    ...
    return found_links[:MAX_STRATEGIC_LINKS]
...
_CLAUDE_MAX_RETRIES = CRAWL_ANALYZE_MAX_RETRIES
_CLAUDE_RETRY_DELAY = CRAWL_ANALYZE_RETRY_DELAY
    ...
    for attempt in range(_CLAUDE_MAX_RETRIES):
        try:
            response = client.messages.create(
                model=LLM_MODEL,
                max_tokens=CRAWL_ANALYZE_MAX_TOKENS,
...
def crawl4ai_analyze(
    ...
    try:
        ...
        homepage_resp = requests.get(
            website_url, headers=HEADERS, timeout=HTTP_TIMEOUT_CRAWL, allow_redirects=True
        )
