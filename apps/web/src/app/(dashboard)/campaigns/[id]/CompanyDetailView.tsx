...
function LmDocxViewer({ emailId, fallback }: { emailId: string; fallback: string }) {
    ...
    return (
        <div className="mt-2 relative">
            {status === 'loading' && (
                <div className="px-4 py-6 text-sm text-slate-400 text-center bg-slate-50 rounded-lg">
                    Chargement de la lettre…
                </div>
            )}
            <div
                ref={containerRef}
                className={`docx-preview-container bg-white rounded-lg border border-slate-100 overflow-auto ${status === 'loading' ? 'hidden' : ''}`}
...
export default function CompanyDetailView({ block, campaignName, onBack }: Props) {
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            {/* ── Fil d'Ariane ─────────────────────────────────────────────── */}
            <div className="px-6 py-3 border-b border-slate-100 flex items-center gap-2 shrink-0 bg-slate-50/60">
                <button
                    onClick={onBack}
                ...
                <span className="text-xs text-slate-400 truncate max-w-[140px]">{campaignName}</span>
                <span className="text-slate-200">/</span>
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {/* En-tête entreprise */}
                <div className="px-8 pt-6 pb-5 border-b border-slate-50">
                ...
                {resultats.length > 0 && (
                    <div className="px-8 py-5 border-b border-slate-50">
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Contacts trouvés</p>
                        <div className="space-y-2">
                            {resultats.map((contact, idx) => {
                                const name = [contact.prenom, contact.nom].filter(Boolean).join(' ')
                                return (
                                    <div key={idx} className="flex items-start gap-3">
                                        <span className="text-xs font-medium text-slate-500 w-20 shrink-0 pt-0.5">
                ...
                <div className="px-8 py-5">
                    ...
                    {draftEmails.length === 0 ? (
                        <p className="text-sm text-slate-400 py-4 text-center">Aucun email en attente</p>
                    ) : (
                        <div className="space-y-3">
                            {draftEmails.map((email, idx) => {
                                ...
                                return (
                                    <div key={email.id} className="border border-slate-100 rounded-xl overflow-hidden bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                                        ...
                                        >
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2 mb-0.5">
                                                    ...
                                                    {email.to && (
                                                        <span className="text-xs text-slate-400 font-mono truncate">{email.to}</span>
                                                    )}
                                                </div>
                                                <p className="text-xs text-slate-500 truncate pl-5">{email.subject}</p>
                                            </div>
                                            <svg
                                                className={`w-3.5 h-3.5 text-slate-300 transition-transform shrink-0 ${isOpen ? 'rotate-180' : ''}`}
                                        ...
                                        {isOpen && (
                                            <div className="border-t border-slate-50 px-5 pb-5 pt-4 space-y-4">
                                                ...
                                                <div>
                                                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Objet</p>
                                                    <p className="text-sm text-slate-800 bg-slate-50 rounded-lg px-3.5 py-2.5">{email.subject}</p>
                                                </div>

                                                <div>
                                                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Corps du mail</p>
                                                ...
                                                {email.generatedLm && (
                                                    <div>
                                                        <button
                                                            ...
                                                            className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wide hover:text-slate-600 transition-colors w-full py-1"
                                                        >
                                                            <svg
